...
describe('Test snyk code', () => {
  ...
  it.each([
    ...
    'One or more of Snyk\'s services may be temporarily unavailable.'],
  ])('When code-client fails in the report flow, throw customized message for %s', async (testName, codeClientError, expectedErrorUserMessage) => {
    jest
      .spyOn(analysis, 'getCodeTestResults')
      .mockRejectedValue(codeClientError);
    ...
    ).rejects.toHaveProperty('userMessage', expect.stringContaining(expectedErrorUserMessage));
  });
